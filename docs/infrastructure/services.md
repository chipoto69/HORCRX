...
4. **Secrets never live in vessel infrastructure by default.** The signing surface can verify signatures; the gateway can serve bundles; neither should become a generic secret store. Sources: `~/wiki/raw/downloads/2026-04-10/root/system-map.md`, `~/wiki/_meta/hermes-stack/hermes-content-os.md`.
...
This split follows the system-map doctrine that the runtime mind, the public interaction plane, and the secret boundary must not collapse into one host process. Sources: `~/wiki/raw/downloads/2026-04-10/root/system-map.md`, `~/wiki/_meta/hermes-stack/hermes-content-os.md`.
